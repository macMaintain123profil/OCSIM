...
class GroupAlianNameDemoVC: BaseDemoVC {
    ...
    override func viewDidLoad() {
        ...
        field.attributedPlaceholder = NSAttributedString(string: "请输群别名", attributes: [NSAttributedString.Key.foregroundColor : UIColor.gray])
        ...
        btn.setTitle("通过群别名加入群聊", for: .normal)
    ...
    @objc func btnClick() {
        ...
        let text = field.text ?? ""
        if text.count == 0 {
            self.view.makeToast("请输群别名", position: .center)
            return
        }
